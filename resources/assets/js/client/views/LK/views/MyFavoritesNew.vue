    ...
    .orders
      template(v-if="dealList && dealList.items && dealList.items.length")
        .order_block_outside(v-for="(bid, ind) in dealList.items", :key="bid.id")
          BidCardShortNew(:bid="bid", :key="ind", :index="ind", type="favorites", @remove="removeBidFavorites")
...
export default {
  ...
  methods: {
    ...
    removeBidFavorites(bidId) {
      this.$parent.loading = true;
      this.dealList.items = this.dealList.items.filter(bid => bid.id !== bidId);
      this.setCount();
      this.$parent.loading = false
    }
  },
