...
Route::prefix('user')->group(function () {
    Route::post('login', 'Client\Api\v1\LoginController@login')->name('api.login');
    ...
    Route::get('profile/{id}', 'Client\Api\v1\LoginController@getProfileFromId')->where('id', '[0-9]+')->middleware('auth:api'); // показать профиль юзера всем
    ...
    Route::prefix('favourites')->middleware('auth:api')->group(function () {
        ...
        Route::get('/count', 'Client\Api\v1\UserSelectedDealsController@count'); // посчитать сколько всего элементов в избранном
    ...
    Route::post('subscribe', 'Client\Api\v1\UserNoticeSubscribeController@subscribeNoticeFromLK')->middleware('auth:api'); // подписка и отписка на оповещения о новых сделка по почте
    
    Route::get('countunreaded', 'Client\Api\v1\ResponseNotificationsController@countUnreaded')->middleware('auth:api'); // не прочитанные отклики
    Route::post('markreaded', 'Client\Api\v1\ResponseNotificationsController@markReaded')->middleware('auth:api'); // сделать отклик прочитанным
});
...
Route::prefix('deals')->group(function () {
   ...
   // Route::get('dealsBeforeTo', 'Client\Api\v1\DealController@runDeleteDealsBeforeToDateWorker');
    ...
    Route::middleware('auth:api')->group(function () {
        ...
        // отклики     
        Route::post('{id}/take', 'Client\Api\v1\DealController@dealsTake'); // участвовать в сделке
        Route::post('{id}/winner', 'Client\Api\v1\DealController@dealsSetWinner'); // выбрать победителя в сделке  
        Route::get('user/responses', 'Client\Api\v1\DealController@getResponsesForCurrentUser'); // отклики текущего юзера
        Route::post('user/responses/{id}/finish', 'Client\Api\v1\DealController@finishResponseCurrentUser'); // выйти из своего отклика
    });

    Route::get('{id}', 'Client\Api\v1\DealController@dealsShow'); // показать заявку по ее id
    Route::get('url/{url}', 'Client\Api\v1\DealController@dealsFromUrlShow'); // показать заявку по ее url
...
//create buy deal landing заказ с лендинга
...
//send email support create deal
Route::post('/send/support/createdeal', 'Client\Api\v1\SupportController@createDeal');
//send email support (inn create org failed)
Route::post('/send/support/innfailed', 'Client\Api\v1\SupportController@innFailed')->middleware('auth:api');
//send email support (deal is failed) - сделка не состоялась
Route::post('/send/support/dealfailed', 'Client\Api\v1\SupportController@dealFailed')->middleware('auth:api');

// finance-operations
Route::group(['prefix' => 'finance-operations', 'as' => 'finance-operation.', 'middleware' => 'auth:api'], function(){
    Route::post('/', 'Client\Api\v1\FinanceOperationController@index')->name('index');
    Route::get('{id}', 'Client\Api\v1\FinanceOperationController@get')->name('get');
    Route::post('store', 'Client\Api\v1\FinanceOperationController@create')->name('create');
    Route::post('{id}', 'Client\Api\v1\FinanceOperationController@update')->name('update');
    // удалять операции вообще не даем пока!!!!!
    //Route::delete('{id}', 'Client\Api\v1\FinanceOperationController@delete')->name('delete');

    // пытаемся оплатить payture
    Route::post('/payture/store', 'Client\Api\v1\FinanceOperationController@paytureStore')->name('payture-store');
});

// сюда идет возврат с payture
Route::group(['prefix' => 'finance-operations', 'as' => 'finance-operation.'], function(){
    Route::get('/payture/store', 'Client\Api\v1\FinanceOperationController@paytureReturnFromPayture')->name('payture-return');
});

Route::group(['prefix' => 'deals-file', 'as' => 'deals-file.', 'middleware' => 'auth:api'], function(){
    Route::get('/', 'Client\Api\v1\DealsFileController@index')->name('index');
    Route::get('{id}', 'Client\Api\v1\DealsFileController@get')->name('get');
    Route::post('store', 'Client\Api\v1\DealsFileController@create')->name('create');
...
Route::group(['prefix' => 'ref/api', 'as' => 'agents.'], function(){
    Route::post('getstatistic', 'Client\Api\v1\Organization\OrganizationStatisticController@getStatistic')->name('get-data-org')->middleware('ip.filter:api');
    Route::post('innisavailable', 'Client\Api\v1\Organization\OrganizationStatisticController@innIsAvailable')->name('inn-is-available')->middleware('ip.filter:api'); // проверить инн, зарегана ли организзация с таким ИНН
});
