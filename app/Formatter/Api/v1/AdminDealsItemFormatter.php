...
class AdminDealsItemFormatter extends Formatter
{
    ...
    public static function format($item)
    {
        try {
            ...
            $slugQuestionsHeaders
                = DealsQuestionsHeaderService::getArrFromIdsArray($questionsIds)
                ->keyBy('id')->toArray();
            ...
            $arToCollect = [
                ...
                'categories'     => $item->categories->map(function(
                    $itm,
                    $key
                ) {
                    $catIcon
                ...
                'imagesDeals'    => $item->imagesDeals->map(function(
                    $itm,
                    $key
                ) {
                    return collect([
                ...
                'favourited' => $curUserId
                    && $item->favouritedFromUsers->search(function(
                        $item,
                        $key
                    ) use ($curUserId) {
                ...
                //     ]);
                // }),
                'questions'  => $item->questions->mapWithKeys(function(
                    $itm,
                    $key
                ) use ($slugQuestionsHeaders) {
                    return collect([
                        $slugQuestionsHeaders[$itm->question_id]['slug'] => [
                            'id'          => $itm->id,
                            'deal_id'     => $itm->deal_id,
                ...
                'members'    => (!$user || !$item->relationLoaded('members'))
                    ? null
                    : $item->members->map(function($itm, $key) {
                        return collect([
                            'organization' => OrganizationItemFormatter::format($itm),
                            'answers'      => $itm['deal_answers'] ?? null,
            ...
            if ($item->user->is_org_created) {
                $arToCollect['organization'] = (!$item->organization)
                    ? null
                    : [
                ...
                $arToCollect['user_categories'] = is_null($item->organization)
                    ? collect([])
                    : $item->organization->categories->map(function(
                        $itm,
                        $key
                    ) {
                        $catIcon
            ...
            if ($type_deal === OrganizationDeal::DEAL_TYPE_BUY) {
                $arToCollect['unit_for_unit']
                    = $item->unit_for_unit; // единица измерения за единицу товара (кг, литр)
                $arToCollect['unit_for_all']
                    = $item->unit_for_all; // единица измерения за полный объем товара (кг, литр)
                $arToCollect['count_unit_in_volume']
                    = $item->count_unit_in_volume; // количество единиц в общем объеме (например объем 5 тонн - кол-во единиц 1000 (в килограммах ,это указано в unit_for_unit))
                $arToCollect['procent']
                    = $item->procent; // наш процент от сделки (смотрим по таблице зависимости процента от бюджета сделки
                $arToCollect['val_for_all']
                    = $item->val_for_all; // количество единиц товара (штук, ящиков...)
                $arToCollect['commission']
                    = $item->commission; // наша комиссия со сделки, в рублях
                $arToCollect['date_of_event']
                    = $item->date_of_event; // дата проведения сделки, тут строка в свободной форме

                $countResponses = $item->countResponsesAdminka();
                $arToCollect['count_response']
                    = $countResponses['count_response'];
                $arToCollect['count_response_moderate']
                    = $countResponses['count_response_moderate'];
            }
