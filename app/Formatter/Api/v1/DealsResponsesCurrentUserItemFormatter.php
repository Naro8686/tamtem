...
class DealsResponsesCurrentUserItemFormatter extends Formatter
{
    ...
    public static function format($item)
    {
        try {
            ...
            $idsDealsBuyContacts
                = $user->idsDealsBuyContacts(); // id объявлений с проплаченными контактами
            ...
            $questionsIds = $item->deal->questions->pluck('question_id')
                ->toArray();
            ...
            if (count($slugQuestionsHeaders) > 0) {
                $answers
                    = $item->answers->mapWithKeys(function($itm, $key) use (
                    $slugQuestionsHeaders
                ) {
                    ...
                    return collect([
                        $slugQuestionsHeaders[$question->question_id]['slug'] => [
                            'id'              => $itm->id,
                            'deal_id'         => $itm->deal_id,
...
//
                    'commission'               => count($idsDealsBuyContacts)
                    > 0 ? $item->deal->commission : 200,
                    ...
                    'questions'       => $item->deal->questions->mapWithKeys(function(
                        $itm,
                        $key
                    ) use ($slugQuestionsHeaders, $answers) {
                        $slug
                            = $slugQuestionsHeaders[$itm->question_id]['slug'];
                        $answer = $answers[$slug] ?? null;
                        return collect([
                            $slug => [
                                'id'          => $itm->id,
                                'deal_id'     => $itm->deal_id,
                                ...
                                'question'    => $itm->question,
                                'is_agree'    => $answer['is_agree'] ?? 0,
                            ]
                        ]);
                    }),
                    'files'           => $item->deal->files->map(function(
                        $itm,
                        $key
                    ) {
                        return collect([
            ...
            if ($type_deal === OrganizationDeal::DEAL_TYPE_BUY and $isPayed) {
                if ($item->deal->user->is_org_created) {
                    $arToCollect['deal']['organization']
                        = (!$item->deal->organization)
                        ? null
                        : [
