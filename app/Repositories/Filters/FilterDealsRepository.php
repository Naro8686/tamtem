...
class FilterDealsRepository
{
    ...
    public static function filter($categories = false, $countries = false, $regions = false, $cities = false,
                                  ...
                                  $status = null, $userId = null,  $payment_status = null, $datePublishedDeal = null, $category = null, $region = null)
    {
        ...
        if (!empty($regions)) $query->whereIn('region_id', $regions);
        if (!empty($countries)) $query->whereIn('country_id', $countries);
        if (!empty($region)) $query->where('region_id', $region);
        if (!empty($categories)) $query->whereIn('organizations_deals_categories.category_id', CategoryRepository::getIncludedTree($categories));
   ...
   //     if (!empty($category)) $query->where('organizations_deals_categories.category_id', $category);  
        ...
        if (empty($datePublishedDeal)) {
            $query->orderBy('organizations_deals.published_at', 'desc');
        }
        else{
            $query->orderBy('organizations_deals.published_at', $datePublishedDeal);
        }
        ...
        if (!empty($withPhoto) and ((int) $withPhoto === 1 or  $withPhoto === true)) {
            ...
            return $query->with(['organization',  'categories', 'cities', 'cities.region', 'imagesDeals', 'user', 'favouritedFromUsers', 'files']);
        }

  //      dd($query->with(['organization',  'categories', 'cities', 'user' , 'imagesDeals']));
        return $query->with(['organization',  'categories', 'cities', 'cities.region',  'user' , 'imagesDeals', 'favouritedFromUsers', 'files']);
    }
