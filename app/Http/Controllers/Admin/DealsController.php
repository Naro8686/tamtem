...
class DealsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $deals = OrganizationDeal::filtredData($request, [
            'id', 'user_id', 'name', 'status', 'budget_from', 'budget_to',
            'budget_all', 'is_fake', 'type_deal', 'finished_at', 'finish',
            'organization_id', 'deadline_deal', 'created_at', 'published_at',
            'planned_finish', 'manager_id', 'moderator_id'
        ]);
        ...
        if (true === $request->has('type_deal')
            and in_array($request->type_deal, [
                OrganizationDeal::DEAL_TYPE_BUY,
                OrganizationDeal::DEAL_TYPE_SELL
            ])
        ) {
    ...
    public function responseDealsIndex(Request $request)
    {
        ...
        $deals = OrganizationDeal::filtredData($request, [
            'id', 'user_id', 'name', 'status', 'type_deal', 'finished_at',
            'finish', 'organization_id', 'deadline_deal', 'created_at',
            'published_at', 'planned_finish', 'manager_id', 'moderator_id'
        ]);
        ...
        foreach ($deals->get() as $deal) {
            ...
            if (isset($curDeal['count_response']) and $curDeal['count_response']
                !== 0
            ) {
                // если стоит - показать только с неотмодерированными откликами
                if ((boolean) $input['only_new'] === true
                    and $curDeal['count_response_moderate'] < 1
                ) {
                    continue;
                }
        ...
        return $this->paginateCollection(collect($dealsresponse),
    ...
    public function responsesByDealIndex(Request $request, $id)
    {
        try {
            ...
            $responses = OrganizationDealMember::with(['organizationWithUser'])
                ->where('deal_id', $id);

            if (!$responses or $responses->count() === 0) {
                return $this->errorResponse("Нет участников в сделке с id = "
                    .$id);
            }

            // показать только на модерации
            if ($request->has('all') and ($request->input('all') === "true"
                    or $request->input('all') === true)
            ) {
                // $responses->where('trading_status', OrganizationDeal::DEAL_TRADING_STATUS_MODERATION);
            } else {
                $responses->where('trading_status',
                    OrganizationDeal::DEAL_TRADING_STATUS_MODERATION);
            }

            return $this->successResponse(AdminResponsesItemFormatter::formatPaginator($responses->paginate($request->input('per_page',
                10))));
        } catch (Throwable $e) {
    ...
    public function getResponse($id)
    {
        try {
            $id = (int) $id;
            $response = OrganizationDealMember::with('answers', 'files')
                ->where('organizations_deals_members.id', $id)->first();
            ...
            return $this->successResponse(AdminResponsesItemFormatter::format($response,
                true));
        } catch (Throwable $e) {
            return $this->errorResponse($e->getMessage());
        }
    ...
    public function updateResponse(ResponseUpdateRequest $request)
    {
        try {
            ...
            if (!$dealsMember) {
                return $this->errorResponse("Нет участника с id = "
                    .$dealsMemberId);
            }

            $price_offer = (true === $request->has('price_offer'))
                ? $request->price_offer : $dealsMember->price_offer;
            $notice = (true === $request->has('notice'))
                ? ($request->get('notice') ?? "") : $dealsMember->notice;
            $is_shipping_included = (true
                === $request->has('is_shipping_included'))
                ? $request->is_shipping_included
                : $dealsMember->is_shipping_included;
            ...
            $organizationDealAnswerService
                = new \App\Services\OrganizationDealAnswer\OrganizationDealAnswerService();
            ...
            foreach ($answers as $slug => $value) {
                ...
                $oldAnswer
                    = $organizationDealAnswerService->getRowIfRowValExists([
                    'deal_id'         => $deal->id,
                    'organization_id' => $dealsMember->organization_id,
                    'question_id'     => $question_id
                ]);
                ...
                if ($oldAnswer) {
                    $organizationDealAnswerService->update($oldAnswer->id,
                        $curAnswer);
                } else {
    ...
    {
        $query = OrganizationDeal::filtredData($request, [
            'id', 'user_id', 'name', 'budget_from', 'budget_to', 'budget_all',
            'is_fake', 'type_deal', 'finish', 'organization_id',
            'deadline_deal', 'created_at', 'published_at', 'manager_id',
            'planned_finish', 'moderator_id'
        ]);

        $query->where('status', OrganizationDeal::DEAL_STATUS_MODERATION);

        $isOnlyMy = ($request->has('only_my_deals')
            and $request->input('only_my_deals') === "true") ? true : false;
    ...
    public function moderateSuccess(Request $request)
    {
        ...
        if ($deal->type_deal === OrganizationDeal::DEAL_TYPE_BUY) {
            ...
            $dqh_specification = isset ($dealQuestion['dqh_specification'])
                ? $dealQuestion['dqh_specification']['question'] : '';
            $dqh_volume = isset ($dealQuestion['dqh_volume'])
                ? $dealQuestion['dqh_volume']['question'] : '';
            $dqh_type_deal = isset ($dealQuestion['dqh_type_deal'])
                ? $dealQuestion['dqh_type_deal']['question'] : '';
            $dqh_min_party = isset ($dealQuestion['dqh_min_party'])
                ? $dealQuestion['dqh_min_party']['question'] : '';
            $unit_for_all = $deal->unit_for_all;

            $isNewDeal = ($deal->created_at->eq($deal->updated_at)) ? true
                : false;
            ...
            if ($isNewDeal) {
                $deal->notify(new UserNewDealBuyMessage('Прошла модерацию',
                    $user,
                    $deal)); // отправит по сокетам оповещения для юзеров, что новая сделка
            } else {
                $deal->notify(new UserNewDealBuyMessage('Была изменена', $user,
                    $deal)); // отправит по сокетам оповещения для юзеров на обновление
            }

            $deal->trading_status
                = OrganizationDeal::DEAL_TRADING_STATUS_WAITING_WINNER;// ожидание победителя

            if ($deal->planned_finish === null
                or (Carbon::parse($deal->planned_finish) > Carbon::now()
                        ->addDays(30))
            ) {
                $deal->planned_finish = Carbon::now()
                    ->addDays(30);// дата окончания сделки
            }
            ...
            if ($tag = (new TagsController())->localCreate($deal->name)) {
                ...
                $organizationToNotify = $tag->organizations()->has('channels')
                    ->with('owner', 'channels')
                ...
                foreach ($organizationToNotify as $curOrg) {
                    ...
                    foreach ($curOrg->channels as $channel) {
                        switch ($channel->name) {
                            // оповещения по почте
                            case 'mail':
                                $urlToUnsubscribe = url('/unsubscribe?mail='
                                    .$curOrg->owner->email."&uid="
                                    .$curOrg->owner->unique_id);
            ...
            $categoriesIds = $deal->categories()->categoriesNotRoot()
                ->pluck('id')
                ->toArray(); // было велено не учитывать категории первого уровня
            // $categoriesIds = $deal->categories()->pluck('id')->toArray();

            // рассылка оповещений о появлении нового заказа в их  категории
            $organizationToNotify = Organization::with([
            ...
            foreach ($organizationToNotify as $key => $orgCats) {
                ...
                if ($user->id === $orgCats->owner->id
                    or count($orgCats->categories) < 1 or
                    $orgCats->owner->is_org_created !== 1
                    or $orgCats->owner->status !== User::USER_STATUS_APPROVE
                ) {
                    continue;
                }
            ...
            $deal->notify(new SendEmailModerate('Объявление опубликовано',
                'Ваша заявка одобрена администратором.', $urlToDeal));
        } else { // если сделка на продажу SELL

            // добавим теги в БД и подпишем юзера на них
            $tags = $request->get('tags', '');
            $newTagsIds
                = (new TagsController())->localCreateTagsFromString($tags);
            ...
            foreach ($newTagsIds as $tagId) {
                ...
                if ($tag = (new TagService())->getRowValue('id', $tagId)) {
                    $tag->organizations()
                        ->syncWithoutDetaching($organization_id);
                }
            }

            $urlToDeal = url('/#!sales/'.$deal->url);

            $deal->notify(new UserNewDealBuyMessage('Прошла модерацию', $user,
                $deal));
            $deal->notify(new SendEmailModerate('Объявление опубликовано',
                'Ваша заявка одобрена администратором.', $urlToDeal));
        }
    ...
    public function moderateFails(Request $request)
    {
        ...
        if ($subscriptionUser) {
            ...
            if ($subscriptionUser->started_at === null) {
                $this->updateBalance($deals->user_id,
                    $subscriptionUser->cost); // вернуть деньги
            }
        }


        $deals->notify(new SendEmailModerate('Модерация отклонена',
            'Ваша заявка отклонена администратором.', url('bids/'.$deals->id)));

        $user = $deals->user()->first();
        $deals->notify(new SendStatusChangeNotification('Ваша заявка отклонена администратором.',
            $user, $deals));
    ...
    public function failModerationResponse(Request $request)
    {
        try {
            ...
            $dealsMember->trading_status
                = OrganizationDeal::DEAL_TRADING_STATUS_BANNED;
            $dealsMember->moderator_id = Auth::user()->id;
            $dealsMember->is_readed_owner_deal
                = true; // хозяин заявки не должен получить уведомление о непрочитанном
            $dealsMember->save();
    ...
    public function successModerationResponse(Request $request)
    {
        try {
            ...
            $dealsMember->is_readed_owner_deal
                = false; // хозяин заявки не прочел новый неотмодерированный
            $dealsMember->moderator_id = Auth::user()->id;
            $dealsMember->save();

            // Начислим балл в статистику, если она есть
            if ($statistic = $dealsMember->organization()->first()->statistic()
                ->first()
            ) {
    ...
    private function getSubscribeUserDeal($deal_id, $userId)
    {
        ...
        $userSubscription = $model->where('deal_id', $deal_id)
    ...
    {
        UserSubscription::where('deal_id', $deal_id)->where('user_id', $userId)
            ->where('status', Subscription::SUBSCRIPTION_STATUS_PAUSE)
            ->update(
                [
    ...
    private function updateBalance($user_id, $cost)
    {
        ...
        $rowFinanceOperation
            = (new \App\Services\FinanceOperation\FinanceOperationService())->storeLocalPayment($user,
            Auth::user(), $cost,
            "Возврат средств по заявке, не прошедшей модерацию");
    }
    ...
    public function generateUniqueOrderIdValue($prefix = null)
    {
        ...
        if ($this->financeOperationService->isRowValueExists('payment_id',
            $orderId)
        ) {
    ...
    public function show($id)
    {
        try {
            ...
            if (!$deal = OrganizationDeal::with([
                'organization', 'members', 'categories', 'cities', 'user',
                'questions', 'winner', 'files'
            ])->find($id)
            ) {
    ...
    public function update(DealUpdateRequest $request, $id)
    {
        ...
        $dealId = $deal->id;
        $isDealSell = ($deal->type_deal === OrganizationDeal::DEAL_TYPE_SELL)
            ? true : false;

        $deal->name = $request->get('name');
        $deal->description = $request->get('description');
        $deal->deadline_deal
            = $this->getNullIsStringNullLetters($request->get('deadline_deal',
            null)); // срок действия заявки
        ...
        if ($isDealSell) {
            ...
            if ($deal->deadline_deal != null) {
                $deal->planned_finish
                    = $deal->deadline_deal; // планируемая дата окончания заявки
            }
            ...
            $deal->unit_for_unit
                = $this->getBlankStringIsStringNullLetters($request->get('unit_for_unit',
                ''));
            $deal->unit_for_all
                = $this->getBlankStringIsStringNullLetters($request->get('unit_for_all',
                ''));
            $deal->count_unit_in_volume = $request->get('count_unit_in_volume',
                0); // количество единиц в общем объеме (например объем 5 тонн - кол-во единиц 1000 (в килограммах ,это указано в unit_for_unit))
            $deal->procent = $request->get('procent',
                0); // наш процент от сделки (смотрим по таблице зависимости процента от бюджета сделки
            $deal->val_for_all = $request->get('val_for_all',
                0);  // количество единиц товара (штук, ящиков...)
            $deal->commission = $request->get('commission',
                0); // наша комиссия со сделки, в рублях
            $deal->date_of_event
                = $this->getBlankStringIsStringNullLetters($request->get('date_of_event',
                '')); // дата проведения сделки, тут строка в свободной форме
            ...
            foreach ($questions as $slug => $value) {
                $question_id
                    = $this->dealsQuestionsHeaderService->getRowValue('slug',
                    $slug);
                if ($question_id === null) {
                    continue;
                }
                ...
                // если есть старое значение, то обновить
                $oldQuestion
                    = $this->dealsQuestionsService->getRowIfRowValExists([
                    'deal_id' => $dealId, 'question_id' => $question_id
                ]);

                if ($oldQuestion) {
                    $this->dealsQuestionsService->update($oldQuestion->id,
                        $curQuestion);
                } else {
        ...
        $deal->moderator_id = Auth::user()->id; // кто модератор
        $deal->url = \App\Classes\Helpers\Translit::translitToUrl($deal->name)
            .'-'.$deal->id;// формириуем урл
        $deal->save();
            ...
            if ($files) {
                ...
                foreach ($files as $file) {
                    if ($file !== null) {
                        ...
                        $dealsFile
                            = $this->dealsFileService->create($toFileService);

                        if (!($dealsFile instanceof DealsFile)) {
                            return $this->errorResponse('Объявление создано, но файл не загружен :'
                                .$dealsFile);
                        }
                    }
                }
    ...
    public function subscriptionToExistTagsAndReturnIsNotExist(Request $request)
    {
        ...
        $arTagsExistAndNotExistInDB
            = (new TagsController())->getArrayExistAndNotExistTagsInDbFromString($string);
        ...
        return $this->successResponse(implode(",",
            $arTagsExistAndNotExistInDB['not_exist']));
    }

    public function uploadImageDeal(Request $request)
    {
        $images = new ImagesDeals();
        $images->uploadFileAndSaveToDB($request->file, 1, $request->deal_id,
            config('b2b.images.resizeValMaxPx'));
    ...
    public function storeDealManager(Request $request)
    {
        $input = $request->all();
        $validator = Validator::make($input, [
            'type_deal'  => 'required|in:'.OrganizationDeal::DEAL_TYPE_SELL.','
                .OrganizationDeal::DEAL_TYPE_BUY,
        ...
        if ($user_id and ($user = User::where('id', $user_id)
                ->first())
        ) { // если привязка автогенеренных сделок к пользователю
            $ids = $this->storeDealsForUser($user, $type_deal, $count_deal,
                $is_fake);
        ...
        if ($ids === false) {
            return $this->errorResponse("Создать сделки в количестве "
                .$count_deal
                ." шт. - не получилось, обратитесь к разработчикам");
        }
    ...
    private function storeAccount($type_deal, $count_deal = 1)
    {
        ...
        $user = new User();
        $user->name = ($type_deal === OrganizationDeal::DEAL_TYPE_SELL)
            ? 'Продавец' : 'Покупатель';
        ...
        $user->account_is_created
            = Auth::user()->email; //User::ACCOUNT_CREATED_AUTO;
        $user->hide_email = 1;
        $user->save();
...
//            dd($organization['categories']);
        ...
        $user->organization()
            ->associate($organization);  // Этот метод обновит отношения belongsTo() и установит внешний ключ на дочерней модели, в частности owner_id = users.id
    ...
    private function generateOrg()
    {
        ...
        $categorySecondLevelId = Category::CategoriesNotRoot()
            ->first(); //id категории , не выше первого уровня
    ...
    private function storeDealsForUser(
        $user,
        $type_deal,
        $count_deal = 1,
        $is_fake = 0
    ...
    private function dealsStore(
        ...
        try {
            ...
            $subtype_deal = ($type_deal === OrganizationDeal::DEAL_TYPE_SELL)
                ? OrganizationDeal::DEAL_SUBTYPE_NEW
                : OrganizationDeal::DEAL_SUBTYPE_NA;
...
//            }
            ...
            $deal->type_deal
                = $type_deal; // тип сделки (сейчас: покупка - buy, продажа - sell)
            $deal->subtype_deal
                = $subtype_deal; // подтип сделки (сейчас: новое - new, бу - used, без подтипа - na)
            ...
            $deal->description
                = 'Здесь должно быть описание сделки';  // описание заявки
            ...
            if ($images) {
                foreach ($images as $image) {
                    if ($image !== null) {
                        $images = new ImagesDeals();
                        if ($images->uploadFileAndSaveToDB($image,
                                $deal->user_id, $deal->id,
                                config('b2b.images.resizeValMaxPx')) === false
                        ) {
