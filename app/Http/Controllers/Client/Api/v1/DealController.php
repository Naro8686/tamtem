...
class DealController extends Controller
{
    ...
    public function dealsList(Request $request)
    {
        ...
        if (!in_array($typeDeal,
            [OrganizationDeal::DEAL_TYPE_SELL, OrganizationDeal::DEAL_TYPE_BUY])
        ) {
            $typeDeal = null;
        }
        $subtypeDeal = $request->input('subtype_deal', null);
        if (!in_array($subtypeDeal, [
        ...
        if (!in_array($status, [
            OrganizationDeal::DEAL_STATUS_MODERATION,
        ...
        try {
            return $this->successResponse(
                ...
                // FilterDealsRepository::getUserDeals($user)->simplePaginate() // todo удалить при чистке
                    FilterDealsRepository::filter($categories, $countries,
                        $regions, $cities, false, false, false, $finish,
                        $typeDeal,
                        false, false, false, false, false, false, false,
                        $subtypeDeal, $status, $userId, $payment_status)
                        ->simplePaginate()
                )
            );
        } catch (Throwable $e) {
    ...
    public function dealsListCurrentUser(Request $request)
    {
        ...
        try {
            return $this->successResponse(
                DealsItemFormatter::formatPaginator(
                    FilterDealsRepository::getUserDeals($user, $categories,
                        $countries, $regions, $regions, $finish, $typeDeal,
                        $subtypeDeal, $status, $payment_status)
                        ->simplePaginate()
                )
            );
        } catch (Throwable $e) {
    ...
    public function dealsStore(DealStoreRequest $request)
    {
        try {
            ...
            $subtype_deal = $request->get('subtype_deal',
                OrganizationDeal::DEAL_SUBTYPE_NA);
            ...
            $isDealSell = ($type_deal === OrganizationDeal::DEAL_TYPE_SELL)
                ? true : false;
            $isProAccount = false;
            ...
            if (!$isDealSell) {
                ...
                if ($deal->deadline_deal != null
                    and (Carbon::parse($deal->deadline_deal) < Carbon::now()
                            ->addDays(30))
                ) {
                    $deal->planned_finish
                ...
                $deal->date_of_event
                    ...
                    'не указано')); // дата проведения сделки, тут строка в свободной форме
            }
            ...
            $deal->url
                = \App\Classes\Helpers\Translit::translitToUrl($deal->name).'-'
            ...
            if (!$isDealSell) {
                ...
                $questions['dqh_doc_confirm_quality']
                    = (isset($questions['dqh_doc_confirm_quality']))
                    ? $questions['dqh_doc_confirm_quality'] : 'не указано';
                $questions['dqh_type_deal']
                    = (isset($questions['dqh_type_deal']))
                    ? $questions['dqh_type_deal'] : '';
                ...
                foreach ($questions as $slug => $value) {
                    ...
                    $oldQuestion
                        ...
                        'deal_id' => $deal->id, 'question_id' => $question_id
                    ]);
            ...
            if ($deal->type_deal === 'sell') {
                if (config()->has('services.telegram-bot-api')
                    && (!empty(config('services.telegram-bot-api.token'))
                        && !empty(config('services.telegram-bot-api.group_id')))
                ) {
                    $deal->notify(new SendTelegramNewDealSell);
                }
                $message = '<a href="'.$notificationLink
                    .'">Есть новое объявление</a>';
...
//				$deal->notify(new SendSlackNewDealCreate);
            ...
            $admins = User::whereIn('role',
                [User::ROLE_ADMINISTRATOR, User::ROLE_MODERATOR])->get();
    ...
    public function dealsUpdate(DealUpdateRequest $request, $id)
    {
        try {
            ...
            $isDealSell = ($deal->type_deal
                === OrganizationDeal::DEAL_TYPE_SELL) ? true : false;
            $isNeedPayDealSell = ($isDealSell and ($deal->status
                    === OrganizationDeal::DEAL_STATUS_ARCHIVE or $deal->status
                    === OrganizationDeal::DEAL_STATUS_BANNED or $deal->finish
                    === true)) ? true : false;
            $isProAccount = false;
            // ============================== Платность услуги объявление о продаже ============================================================
            ...
            if ($isNeedPayDealSell) {
                $paymentInfo
                    = $this->paymentService->paymentServiceIsPossible($request,
                    Subscription::SUBSCRIPTION_PAYMENT_ONCE_DEAL_SELL, true);
                $isProAccount = $paymentInfo['is_pro_account'];
            ...
            if (!$isDealSell) {
                ...
                $deal->budget_to = $request->get('budget_to',
                    0); // бюджет до (сейчас просто - бюджет)
            ...
            $deal->is_need_kp = $request->get('is_need_kp',
                0); // надо ли коммерческое предложение требовать с другой стороны
            ...
            if ($isNeedPayDealSell) {
                if ($isProAccount) {
                    $this->paymentService->newUserSubscription($paymentInfo['subscription_id'],
                        $user->id, $deal->id, $paymentInfo['service_name']
                        .". Pro аккаунт. Восстановление сделки", null, null,
                        $isProAccount);
                } else {
                    $user->ballance -= $paymentInfo['cost_of_service'];// снимем сразу сумму за  услугу
                    $user->save();
                    $user_subscription_id
                        = $this->paymentService->newUserSubscription($paymentInfo['subscription_id'],
                        $user->id, $deal->id,
                        $paymentInfo['service_name'].". Восстановление сделки",
                        null, null, $isProAccount);
                    // создадим запись в финансовых операциях
                    $rowFinanceOperation
                        = (new \App\Services\FinanceOperation\FinanceOperationService())->storeLocalPayment($user,
                        null, -$paymentInfo['cost_of_service'],
                        "Снятие средств за: ".$paymentInfo['service_name']
                        .". Восстановление сделки", $user_subscription_id);
                }
            }
            // ==============================END  Платность услуги рассылки оповещений по эмайл ============================================================
            ...
            // перенесено в App\Http\Controllers\Admin\DealsController    moderateSuccess()
            $deal->notify(new UserNewDealBuyMessage('goto_moderation_with_update',
                $user, $deal)); // отправит по сокетам оповещения для юзеров
    ...
    public function dealsShow($id)
    {
        try {
            ...
            if (!$deal = OrganizationDeal::with([
                'organization', 'categories', 'cities', 'user', 'questions',
                'files'
            ])->find($id)
            ) {
    ...
    public function dealsFromUrlShow($url)
    {
        try {
            ...
            // если юзер не авторизован и заказ завершен - не показывать
            if (!$user and ($deal->status
                    === OrganizationDeal::DEAL_STATUS_ARCHIVE
                    or $deal->trading_status
                    === OrganizationDeal::DEAL_TRADING_STATUS_FINISHED)
            ) {
                return $this->errorResponse();
            }
            ...
            $isDealMy = false; // моя ли сделка
            $isTypeDealBuy = ($deal->typeDeal
                === OrganizationDeal::DEAL_TYPE_BUY) ? true : false;
            $isDealActive = ($deal->status
                === OrganizationDeal::DEAL_STATUS_APPROVE) ? true : false;
            ...
            if ($user && $deal->organization_id == $user->organization_id) {
                ...
                $isDealMy = true;
            } elseif ($isTypeDealBuy === true && $deal->isUserMemberDeal($user)
                && $isDealActive === true
    ...
    public function dealsTake(DealTakeRequest $request, $id)
    {
        ...
        try {
            ...
            $deal->members()->attach($user->organization_id, [
                ...
                'price_currency'       => $request->get('price_currency',
                    'rur'),
                'is_shipping_included' => $request->get('is_shipping_included',
                    false),
                'notice'               => $request->get('notice') ?? "",
            ]);

            $memberId = OrganizationDealMember::where('deal_id', $dealId)
            ...
            if ($images) {
                foreach ($images as $image) {
                    ...
                    // если сам юзер загружает файл или админ за него
                    $responseFile
                        = $this->responsesFileService->create($toFileService);
...
//		$deal->notify(new SendSlackNewResponseForManager);
    ...
    public function dealsSetWinner(Request $request, $id)
    {
        ...
        if ((!$deal = OrganizationDeal::find($id))
            || ($user->organization_id != $deal->organization_id)
        ) {
            throw new \App\Exceptions\NotFoundException();
        }
        ...
        if (!$winner = $deal->membersFromOrganizationMembersTable()
            ->where('id', $request->get('winner_id'))->first()
        ) {
            return $this->errorResponse();
        }

        // проставили участнику - ожидание оплаты
        $winner->trading_status
            = OrganizationDeal::DEAL_TRADING_STATUS_WAITING_PAYMENT;
        $winner->is_readed_owner_response = false;
        $winner->save();

        $deal->winner_id
            = OrganizationDealMember::find($request->get('winner_id'))->organization_id;
        $deal->trading_status
            = OrganizationDeal::DEAL_TRADING_STATUS_WAITING_PAYMENT;
        $deal->save();
        ...
        $idsDealsBuyContacts
            = $winnerUser->idsDealsBuyContacts(); // id объявлений с проплаченными контактами
        ...
        if ($isPayed) {
            ...
            $deal->membersFromOrganizationMembersTable()
                ->update(['trading_status' => OrganizationDeal::DEAL_TRADING_STATUS_FINISHED]);
            $winner->is_readed_owner_response
                = false; // оплатили, значит откик в не прочитанные
            $winner->save();
            ...
            $deal->notify(new DealWinner("Прошла оплата получения контактов сделки в размере "
                .round($commission, 0, PHP_ROUND_HALF_DOWN).
                " руб. из вашего кошелька. ".PHP_EOL.PHP_EOL
                ." Теперь вы можете общаться с вашим партнером по сделке."));

            // надо послать победителю письмо, что его выбрали и все оплачено
            $deal->notify(new DealWinnerResponse(url('bids/'.$deal->id),
                round($commission, 0, PHP_ROUND_HALF_DOWN)));


            // надо создать диалог по данной сделке между продавцом и покупателем и послеть первое сообщение
            if (!$dialogId = DialogsController::newLocalDialog($deal->id,
                $deal->winner_id, $deal->organization_id)
            ) {
            ...
            if (!$messageId = DialogsController::sendLocal($dialogId, $user,
                $message)
            ) {
            ...
            $deal->notify(new DealWinner("Чтобы получить контакты по сделке, необходимо оплатить "
                .round($commission, 0, PHP_ROUND_HALF_DOWN).
                ...
                .
                "<p><a style='color: #ffffff; display: block; padding: 10px 15px; border-radius: 32px; background-color: #2fc06e; font-family: Helvetica, Arial, sans-serif; font-size: 14px; font-weight: 600; line-height: 15px; text-decoration: none;text-align: center; width: 200px; margin: 0 auto;' href="
                .url('bids/'.$deal->id).">Оплатить заказ</a></p>"
            ));

            //	Mail::to(config('b2b.email.callme'))->send(new SendToManagers( '+7' . $request->get('phone')));
            $messageToManagers = "Выявлен победитель сделки №".$deal->id." ("
                .$deal->name.") - это пользователь: ".$winnerUser->name
                .", с телефоном: +7".$winnerUser->phone.
            ...
            Mail::to(config('b2b.email.managers'))
                ->send(new SendToManagers((string) $messageToManagers));
...
//		$deal->notify(new SendSlackChooseWinner);
    ...
    public function dealsFinish(Request $request, $id, $isLocalRequest = false)
    {
        try {
            ...
            $deal->notify(new UserNewDealBuyMessage('deleted', $user,
                $deal)); // отправит по сокетам оповещения для юзеров
    ...
    {
        $dealsWaitingPaymentResponses = OrganizationDeal::where('finish', '=',
            false)->with('membersFromOrganizationMembersTable')
            ->whereDate('winner_wating_payment_at', '<',
                Carbon::now()) // вышла крайняя дата оплаты
            ->whereNotNull('winner_id')
            ->where('trading_status', '=',
                OrganizationDeal::DEAL_TRADING_STATUS_WAITING_PAYMENT)
        ...
        foreach ($dealsWaitingPaymentResponses->get() as $key => $deal) {
            ...
            foreach ($deal->membersFromOrganizationMembersTable as $curMember) {
                if ($curMember->organization_id
                    === $curWinnerId
                ) { // если победитель, то проставляем ему banned ибо не заплатил
                    $curMember->trading_status
                        = OrganizationDeal::DEAL_TRADING_STATUS_BANNED;
    ...
    public function finishDealsAfterPlannedFinish()
    {
        ...
        return $dealsToDelete->update([
            'finish' => true, 'finished_at' => $nowDate,
    ...
    public function needRefreshDealsBeforeSpecifiedDays($beforeDays = 5)
    {
        ...
        $idsDealsTo5DaysBeforeDelete = OrganizationDeal::where('finish', '=',
            false)
            ->where(DB::raw('DATE_FORMAT(planned_finish,"%Y-%m-%d")'), '=',
                $nowDate->addDays($beforeDays)->format('Y-m-d'))
            ->where('status', '=', OrganizationDeal::DEAL_STATUS_APPROVE)
            ->orderBy('published_at', 'DESC')
            ->pluck('id')->toArray();

        if (count($idsDealsTo5DaysBeforeDelete) > 0) {
            $messageToManagers = 'Заказы с номерами: '.implode(",",
                    $idsDealsTo5DaysBeforeDelete)." надо продлить.";
            $title = 'Необходимо продлить заказ';
            $subject = 'Отделу сопровождения надо продлить заказ';
            Mail::to(config('b2b.email.managers'))
                ->send(new SendToManagers((string) $messageToManagers, $title,
                    $subject));
        }
    ...
    public function runDeleteDealsBeforeToDateWorker()
    {
        try {
            ...
            // остановим подписки
            $SubsciptionToFinished = UserSubscription::where('status', '<>',
    ...
    public function deletedDealsList(Request $request)
    {
        ...
        try {
            $ret = $this->successResponse(
                DealsItemFormatter::formatPaginator(
                    FilterDealsRepository::filter(false, false, false, false,
                        false, false, false, 1, null,
                        false, false, false, false, false, false, false, null,
                        $status, $userId)->simplePaginate()
                )
            );
        ...
        try {
            ...
            if ($isDealSell) {
                if ($isProAccount) {
                    $this->paymentService->newUserSubscription($paymentInfo['subscription_id'],
                        $user->id, $deal->id, $paymentInfo['service_name']
                        .". Восстановленная из архива. Pro аккаунт", null, null,
                        $isProAccount);
                } else {
                    $user->ballance -= $paymentInfo['cost_of_service'];// снимем сразу сумму за  услугу
                    $user->save();
                    $user_subscription_id
                        = $this->paymentService->newUserSubscription($paymentInfo['subscription_id'],
                        $user->id, $deal->id, $paymentInfo['service_name']
                        .". Восстановленная из архива.", null, null,
                        $isProAccount);
                    // создадим запись в финансовых операциях
                    $rowFinanceOperation
                        = (new \App\Services\FinanceOperation\FinanceOperationService())->storeLocalPayment($user,
                        null, -$paymentInfo['cost_of_service'],
                        "Снятие средств за: ".$paymentInfo['service_name']
                        .". Восстановленная из архива.", $user_subscription_id);
                }
            }
    ...
    public function getResponsesForCurrentUser(Request $request)
    {
        ...
        try {
            ...
            $responses = OrganizationDealMember::with([
                ...
                ->with('deal.questions', 'deal.questions.questionHeader',
                    'deal.files');

            // отбросим те, которые завершены и не мои
            $toPaginate = $responses->orderBy('id', 'desc')->get();
            $toPaginate = $toPaginate->map(function($item, $key) use (
                $organization_id
            ) {
                if ($item->trading_status
                    === OrganizationDeal::DEAL_TRADING_STATUS_FINISHED
                    and ($item->deal->winner_id === null
                        or $item->deal->winner_id !== $organization_id)
                ) {
                    $item = null;
                }
                return $item;
            ...
            $paginateCollection
                = $this->paginateCollectionNotArrayToPaginator($toPaginate,
     ...
     * @return void
     */
    public function finishResponseCurrentUser(
        Request $request,
        $id,
        ...
        try {
            $organization_id = $user->organization_id;
            $response = OrganizationDealMember::where('id', '=', $id)
                ->where('organization_id', '=', $organization_id)
                ->with('answers', 'deal')->first();
            ...
            if ($response->deal->winner_id === $organization_id) {
                if ($response->deal->trading_status
                    === OrganizationDeal::DEAL_TRADING_STATUS_WAITING_PAYMENT
                ) {
                    $response->deal->trading_status
