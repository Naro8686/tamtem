...
class DialogsController extends Controller
{
    ...
    public function dialogs(Request $request)
    {
        try {
            ...
            if ($organization) {
                $dialogs = $organization->dialogs_all()->select([
                    'id', 'seller_id', 'buyer_id', 'deal_id', 'created_at'
                ])
                    ->orderBy('updated_at', 'desc')
                    ->with('seller', 'buyer', 'deal', 'last_message')
                    ->simplePaginate(15);
            } else {
                $dialogs = new Paginator([], 15);
            }
            $formatPaginator = new DialogItemFormatter();

            return $this->successResponse($formatPaginator->formatPaginator($dialogs,
                $myOrg, $myUserId));
        } catch (\Exception $e) {
    ...
    public function countunreaded(Request $request)
    {
        try {
            ...
            if (!is_null($organization)) {
                $dialogs = $organization->dialogs_all()->get();
            }
            $countUnreaded = 0;
    ...
    public function markReaded(Request $request)
    {
        try {
            ...
            $message = Message::where("id", "=", $messageId)
                ...
                ->where("status", "=", Message::MESSAGE_STATUS_UNREADED)
                ->first();
    ...
    public function dialog(Request $request, $id)
    {
        try {
            ...
            if ($user->organization_id != $dialog->seller_id
                && $user->organization_id != $dialog->buyer_id
            ) {
            ...
            $dealInfo = $dialog->deal()
                ->select('id as deal_id', 'name as deal_name')->first()
                ->toArray();

            $messages = $dialog->messages()->with('user.organization', 'files')
    ...
    public function newDialog(Request $request)
    {
        try {
            ...
            if (!$member = $deal->members()
                ->where('organization_id', $member_id)->first()
            ) {
            ...
            ) {  // тот, кому мы пишем - продавец
                $buyer_id = $user->organization_id;
                $seller_id = $org_to;
            } elseif ($deal->type_deal
            ...
            if (!$dialog = Dialog::where('deal_id', $deal_id)
                ->where('buyer_id', $buyer_id)->where('seller_id', $seller_id)
                ->first()
            ) {
                $dialog = new Dialog();
     ...
     */
    public static function newLocalDialog(
        $dealId = null,
    ...
    public function send(Request $request, $id)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'budget_to' => 'numeric',
                'type'      => 'in:'.Message::MESSAGE_TYPE_MESSAGE.','
                    .Message::MESSAGE_TYPE_RESPONCE,
            ...
            if ($type === Message::MESSAGE_TYPE_RESPONCE) {
                ...
                if ($is_need_kp
                    and !$request->hasFile('files')
                ) { // если коммерческое предложение обязательно при отклике
            ...
            if ($files) {
                foreach ($files as $file) {
                    if ($file !== null) {
                        ...
                        if ($files->uploadFileAndSaveToDB($file, $message->id,
                                $dialog->id, $dialog->deal->id, $user->id)
                            === false
                        ) {
            ...
            if ($type === Message::MESSAGE_TYPE_RESPONCE) {
                ...
                $resultPayment = $payment->paymentSubscription($request,
                    Subscription::SUBSCRIPTION_PAYMENT_ONCE_DEAL_BUY_GET_CONTACTS,
                    true, $dialog->deal->id);
                if ($resultPayment
                    === false
                ) { // оплата не прошла - удалим отклик
                    $message->delete();
                    return $this->errorResponse('Ошибка оплаты: '
                        .Subscription::SUBSCRIPTION_PAYMENT_ONCE_DEAL_BUY_GET_CONTACTS);
                }
            }
            $user->organization->notify(new \App\Notifications\OrganizationChatMessage($message,
                $user));
    ...
    // mvp tamtem
    // послать сообщение в диалог  ======================================================
    public static function sendLocal(
        $dialogId,
        $userFrom,
        $message = '',
        $type = Message::MESSAGE_TYPE_MESSAGE
    ) {
        try {
